...
class SparseBM25Index:
    ...
    def _tokenize(self, text: str) -> List[str]:
        ...
        # FIXED: Improved tokenization with punctuation handling and stopwords
        text = text.lower()
        # Remove extra whitespace and newlines
        text = re.sub(r'\s+', ' ', text).strip()
        # Split on non-alphanumeric characters but keep hyphenated words
        tokens = re.findall(r'\b[a-z0-9]+(?:-[a-z0-9]+)*\b', text)
        # Filter out very short tokens and common stopwords
        stopwords = {'the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'of', 'is', 'are', 'be', 'by'}
        ...
        if os.path.exists(self.storage_path):
            try:
                with open(self.storage_path, "rb") as f:
                    ...
                    logger.info(f"Sparse index loaded: {len(self.indexed_chunks)} chunks")
            except (pickle.UnpicklingError, EOFError, KeyError) as e:
                logger.error(f"Failed to load sparse index: {str(e)}")
        else:
            logger.info("No existing sparse index found at {self.storage_path}")